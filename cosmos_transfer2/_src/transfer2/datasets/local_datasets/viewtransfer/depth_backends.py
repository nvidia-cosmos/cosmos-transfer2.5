...
def _generate_agibot_depth(
    *,
    ...
    return np.stack(depth_frames, axis=0, dtype=np.float64)
...
def _generate_moge_depth(
    *,
    ...
    return output["depth"].detach().cpu().double().numpy()
...
def generate_depth(
    *,
    ...
    assert depth_frames.dtype == np.float64, f"Expected depth frames to be float64 in meters, got {depth_frames.dtype}"
    return depth_frames
...
def _run_batch_moge_estimation(rgb_np_uint8, batch_size=8, model=None, device="cuda"):
    ...
    outputs = []
    with tqdm(total=len(input), desc="Running MoGe estimation", unit="frame") as pbar:
